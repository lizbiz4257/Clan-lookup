...
async function loadDayCorrections() {
  ...
  try {
    // Using the GitHub API directly instead of raw.githubusercontent.com —
    // the raw URL is served through a CDN that can cache stale content for
    // several minutes after a commit, which was causing corrections to not
    // show up immediately. The API read is fast and reliable enough for
    // this file's size.
    ...
    dayCorrectionsCache = JSON.parse(decoded);
  } catch (err) {
    dayCorrectionsCache = {};
  }
  return dayCorrectionsCache;
