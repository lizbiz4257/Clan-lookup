...
// which family clan they're currently in (if any) and, if not currently in
// any, which family clan they most recently appeared in (based on war history).
// This lets us answer "what family clan were they in last" using data we
// already have, instead of an extra live lookup per departed player.
async function buildFamilyIndex() {
  const perClan = await Promise.all(FAMILY_TAGS.map(async (tag) => {
    ...
    return { tag, name: clanInfo.name, currentMembers, lastSeenWeek };
  }));
