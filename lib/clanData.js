...
async function lookupOneClan(tag, familyIndex) {
  ...
  const rows = memberTagList.map((playerTag) => {
    ...
    const lastWeek = h.byWeek[0];
    const thisWeekScore = cur ? cur.score : (lastWeek ? lastWeek.score : null);
    const thisWeekAttacks = cur ? cur.attacks : (lastWeek ? lastWeek.attacks : null);
    ...
    // Prior clan history: pulled for anyone with no history in THIS specific
    // clan (whether they're a brand new joiner here, or someone who's not
    // even a current member — just showing up via family cross-referencing).
    // Same league-match requirement applies either way.
    const hasNoLocalHistory = h.scores.length === 0;
    const isNew = isCurrentMember && hasNoLocalHistory;
    let priorClan = null;
    if (hasNoLocalHistory && familyIndex) {
      // New joiners: require the same trophy league. Everyone else missing
      // local history (e.g. not currently a member here at all): no league
      // restriction, so cross-tier movement still shows up.
      priorClan = resolvePriorClanHistory(playerTag, familyIndex, tag, clanInfo.clanWarTrophies, isNew);
    }
