...
async function lookupOneClan(tag, familyIndex) {
  ...
  races.forEach((race, weekIndex) => {
    ...
    // have gotten more than 12 attacks that week — the highest attack count
    // anyone actually hit tells us the real ceiling for that week. To keep
    // "missed attacks" meaningful against a consistent scale of 16, we add
    // back the attacks that were never possible (the phantom 4th day) for
    // everyone equally — so someone who did 11/12 shows as 15/16 (missed 1),
    // not as if they'd missed 5.
    const weekMaxAttacks = participants.length ? Math.max(...participants.map((p) => p.decksUsed)) : 16;
