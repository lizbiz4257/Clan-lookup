...
async function loadDayCorrections() {
  ...
  try {
    ...
    if (!resp.ok) {
      // Don't cache a failure — return empty for THIS call only, so the
      // next request tries again instead of being stuck empty forever.
      return {};
    }
    const json = await resp.json();
    const decoded = Buffer.from(json.content, 'base64').toString('utf-8');
    dayCorrectionsCache = JSON.parse(decoded); // only cache real, successful data
  } catch (err) {
    return {};
  }
...
// Note: the API's sectionIndex is 0-indexed internally, but the "134-1"
// notation (matching CWStats and your own labeling) is 1-indexed — so we
// add 1 here to line up correctly (confirmed against real data: the API's
// sectionIndex 0 for season 134 is what's actually labeled "134-1").
function raceWeekLabel(race) {
  if (race.seasonId != null && race.sectionIndex != null) {
    return race.seasonId + '-' + (race.sectionIndex + 1);
  }
  return null;
}
...
async function buildFamilyIndex() {
  const perClan = await Promise.all(FAMILY_TAGS.map(async (tag) => {
    ...
    let races = [];
    try {
      const log = await royaleApiGet('/clans/' + encTag + '/riverracelog?limit=6');
      races = (log.items || []).slice(0, 6);
...
// finds their most recent OTHER family clan and returns their 6-week score
...
async function lookupOneClan(tag, familyIndex) {
  ...
  const log = await royaleApiGet('/clans/' + encTag + '/riverracelog?limit=6');
  const races = (log.items || []).slice(0, 6);
  ...
  const rows = memberTagList.map((playerTag) => {
    ...
    // "Score This Week"/"Cards Played" stay blank when there's no active war
    // (by design) — but 5wa and Elo need SOMETHING to calculate from even
    // during an off period, so they fall back to the most recent completed
    // week's data internally, without changing what's shown in those two
    // columns themselves.
    const thisWeekScore = cur ? cur.score : null;
    const thisWeekAttacks = cur ? cur.attacks : null;
    const mostRecentWeek = h.byWeek[0];
    const calcScore = thisWeekScore != null ? thisWeekScore : (mostRecentWeek ? mostRecentWeek.score : null);
    const calcAttacks = thisWeekAttacks != null ? thisWeekAttacks : (mostRecentWeek ? mostRecentWeek.attacks : null);
    ...
    // 5wa = score ÷ attacks (this week if a war's active, otherwise the most recent completed week)
    const fivewa = (calcScore != null && calcAttacks) ? Math.round((calcScore / calcAttacks) * 100) / 100 : null;
    ...
    const localWeeks = [0, 1, 2, 3, 4, 5].map((i) => h.byWeek[i] || null);
    ...
    const needsFill = oldestLocalIndex < 5; // there's room for older weeks to be filled in

    const isNew = isCurrentMember && oldestLocalIndex === -1; // truly zero local history anywhere — drives the "New?" column
    const isRecentJoiner = isCurrentMember && needsFill; // still building up their first 6 weeks here
    ...
    if (priorClan && priorClan.priorWeeks) {
      for (let i = oldestLocalIndex + 1, j = 0; i < 6 && j < priorClan.priorWeeks.length; i++, j++) {
    ...
    const row = {
      ...
      fiveKPPG,            // 5kPPG
      weeks                // full 6-week breakdown, blended with prior clan if needed
    };
